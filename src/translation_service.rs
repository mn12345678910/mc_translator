//! # 翻譯服務模組
//! 包含所有翻譯 API 的呼叫邏輯：Gemini、OpenAI、Ollama、免費 Google 翻譯。
//! 支援單筆翻譯與批量翻譯模式。

use crate::translation_job::JobConfig;
use crate::utils::TermType;
...
pub fn build_system_prompt(
    base_prompt: &str,
    glossary: Option<&[crate::utils::GlossaryEntry]>,
) -> String {
...
pub async fn translate_batch(
    ...
    let result = crate::data_processing::validate_and_cleanup(&result);
                    ...
                    for (idx_str, trans_val) in obj {
                            ...
                            (idx_str.parse::<usize>(), trans_val.as_str())
                        {
                            if idx >= 1 && idx <= texts.len() {
                                let cleaned = crate::data_processing::validate_and_cleanup(trans_s);
    ...
    for cap in BATCH_INDEX_RE.captures_iter(&result) {
        if let (Ok(idx), Some(translated)) = (cap[1].parse::<usize>(), cap.get(2)) {
            if idx >= 1 && idx <= texts.len() {
                let translated_text =
                    crate::data_processing::validate_and_cleanup(translated.as_str());
...
fn log_llm_communication(prompt: &str, response: &str) {
    use std::io::Write;
