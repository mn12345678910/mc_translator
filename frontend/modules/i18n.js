...
export async function loadUiLangs() {
    ...
    try {
        ...
        allLangs.forEach(l => {
            ...
            opt.value = l;
            opt.textContent = l === 'zh_tw' ? (state.currentLabels.lang_zh_tw || '繁體中文 (zh_tw)') : 
                              l === 'en_us' ? (state.currentLabels.lang_en_us || 'English (en_us)') : 
                              l === 'zh_cn' ? (state.currentLabels.lang_zh_cn || '简体中文 (zh_cn)') : 
                              l === 'ja_jp' ? (state.currentLabels.lang_ja_jp || '日本語 (ja_jp)') : l;
...
export async function updateUiLanguage() {
    ...
    try {
        ...
        const optionMapping = {
            ...
            'palette-property': { 'bg': labels.label_bg_color, 'text': labels.label_text_color, 'rounding': labels.label_custom_rounding }
        };
        ...
        if (userPrompt && labels.default_user_prompt) {
            ...
            if (knownUserDefaults.some(v => normValue(v) === currentVal)) { userPrompt.value = labels.default_user_prompt; }
        }
        if (systemPrompt && labels.default_system_prompt) {
            const currentVal = normValue(systemPrompt.value);
            if (knownSysDefaults.some(v => normValue(v) === currentVal)) { systemPrompt.value = labels.default_system_prompt; }
        }
    } catch (err) {
