...
>    - **遷移邏輯 (Migration Mode)**：對「官方建議詞」進行任何編輯或取代操作後，該條目必須「移入」使用者字典 (`user.json`) 並從官方字典 (`official.json`) 中移除。
  - **設定與持久化規範**:
    ...
    - **狀態持久化**: 所有關鍵 UI 狀態（包括 API 設定、開發者模式、建議詞管理器開關及自定義 FPS）與視窗幾何資訊均需同步至 `AppConfig` 並持久化。
  ...
  - **UI 視覺規格與連動**:
    ...
    - **進度條動畫**: 運行中必須具備脈衝 (Shimmer) 動畫。顏色需動態連動主題的 `selection.bg_fill`。
    - **配色同步化**: 淺色模式背景使用暖橘色 (#FFFDF0)，文字與標籤統一使用 **深灰色 (#1A1A1A)**。API 指示燈需依主題切換深/淺語義色。
    - **日誌增強**: 所有日誌輸出必須帶有 `[HH:MM:SS]` 時間戳記。
    - **語義化日誌顏色**: 
      - **錯誤 (Err/失敗)**: 強制顯示為 **紅色 (Red)**。
      - **成功 (Done/完成)**: 強制顯示為 **綠色 (Green)**。
