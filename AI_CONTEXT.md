...
>    - **遷移邏輯 (Migration Mode)**：對「官方建議詞」進行任何編輯或取代操作後，該條目必須「移入」使用者字典 (`user.json`) 並從官方字典 (`official.json`) 中移除。
  ...
  - **UI 視覺規格與連動**:
    ...
    - **自定義調色盤 (Palette)**：用戶可針對特定元件（按鈕、標籤、輸入框、進度條）自定義背景、文字顏色與**個別圓角 (Revision 15.30)**。
  - **樣式快照同步 (StyleSnapshot)**：字典管理器等子視窗採用 `StyleSnapshot` 快照機制，確保與主視窗視覺風格即時連動。
  - **進度條自定義**：支援分別調整「目前檔案」與「總進度條」的色調與圓角。
