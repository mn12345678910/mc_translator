...
- **高穩定性流水線**：自動運作 **三級降級重試系統**（全批次 ➡ 切半小批次 ➡ 單筆降級），最大化翻譯成功率；僅本機 Ollama 逾時會啟動安全中斷保護
...
### 增量翻譯與術語輔助
- **增量翻譯 (Incremental)**：自動跳過已翻譯或已存在於目標檔案中的條目，加速後續翻譯並降低重複消耗
- **術語表輔助 (Glossary)**：支持將對應關係動態注入 LLM 系統提示詞（System Prompt），輔助模型保持專有名詞、方塊名稱等技術性術語的一致性
- **進階字典管理器**：
  - 🔍 搜尋：即時過濾原文或翻譯
  - 📄 分頁：每頁 50 筆，支援翻頁導航
  - ✏ 編輯：直接修改翻譯內容
...
- **停止**：可隨時中止翻譯流程，在 Ollama 逾時或其他嚴重錯誤時，會立刻自動中斷保護本機

### 進度追蹤
- **總進度條**：顯示已完成檔案數佔總數的比例（n/total [X%]）
- **即時日誌**：使用 TextEdit 高效能顯示
